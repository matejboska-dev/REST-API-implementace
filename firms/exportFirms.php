...
if(isset($_POST["ids"])){
  ...
  if ($result->num_rows > 0) {
    ...
    $sql = "SELECT  firm.id,firm.name,firm.active,firm.source,firm.date_of_contact,firm.date_of_2_contact,firm.date_of_meeting,firm.result,firm.workshop,firm.brigade,firm.practice,firm.cv,firm.note,subject.name as subject,
firm_contacts.surname as surname, firm_contacts.email as email, firm_contacts.phone as phone 
FROM firm inner join subject on firm.subject_id = subject.id  inner join firm_contacts on firm.id = firm_contacts.firm_id  
WHERE main=1 AND  firm.id in (";
